use factorio_blueprint::{blueprint_string_to_model, model_to_pretty_json};
...
fn main() -> factorio_blueprint::Result<()> {
    let model = blueprint_string_to_model(_SAMPLE_BLUEPRINT)?;
    let reencoded = model_to_pretty_json(model)?;
