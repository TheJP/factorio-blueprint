use factorio_blueprint::{blueprint_string_to_model, model_to_blueprint_string};
use clap::{App, Arg};
...
fn parse_arguments() -> Option<String> {
    ...
    let download = matches.subcommand_matches("reencode")?;
    let blueprint = download.value_of("blueprint-string")?;

    Some(blueprint.into())
}

fn main() -> factorio_blueprint::Result<()> {
    let blueprint = parse_arguments().unwrap();
